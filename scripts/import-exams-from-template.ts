 ...
 * Usage: npx tsx scripts/import-exams-from-template.ts <path-to-file.xlsx>
 */
...
function parseExcelFile(filePath: string): ExamRecord[] {
  ...
  // Check if there's a "Courses" label row above the main header
  if (headerRowIndex > 0) {
    const prevRow = rawData[headerRowIndex - 1];
    if (prevRow && String(prevRow[2] || '').trim().toUpperCase() === 'COURSES') {
      courseHeaderRowIndex = headerRowIndex - 1;
    }
  }
...
async function main() {
  ...
  if (!filePath) {
    console.error('Usage: npx tsx scripts/import-exams-from-template.ts <path-to-file.xlsx>');
    console.error('\nExpected format:');
