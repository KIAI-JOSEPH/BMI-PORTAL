...
import { 
  Search, Printer, Download, ShieldCheck, User, CreditCard,
  Hash, Calendar, MapPin, Phone, Mail, Sparkles, QrCode,
  Fingerprint, BadgeCheck, Contact, IdCardIcon
} from 'lucide-react';
...
export const IDCard: React.FC<IDCardProps> = ({ students, logo }) => {
  ...
  const [generatedCard, setGeneratedCard] = useState<StudentIDCardType | null>(null);
  ...
  const generateCardData = useCallback(async (student: Student): Promise<{ card: StudentIDCardType; security: DocumentSecurityFeatures }> => {
    ...
    const cardData: Omit<StudentIDCardType, 'id' | 'security' | 'createdAt' | 'updatedAt' | 'type'> = {
      ...
      bloodGroup: 'O+',
      emergencyContact: student.phone || '+254 XXX XXX XXX',
      status: 'active',
      createdBy: 'admin'
    };

    const security = await documentService.generateSecurityFeatures('id_card', student.id, cardData, {
      includeBlockchain: false
    });

    const card = await documentService.createDocument<StudentIDCardType>('id_card', student.id, cardData, {
      createdBy: 'admin'
    });
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-slate-500">Generate secure student identification cards</p>
            </div>
          </div>
            ...
            {filteredStudents.map((student) => (
              <div
                ...
                className={`bg-white rounded-xl p-4 cursor-pointer transition-all hover:shadow-lg border-2 ${
                  selectedStudent?.id === student.id ? 'border-violet-500 shadow-lg' : 'border-slate-100 hover:border-slate-200'
                }`}
              >
            ...
            <div className={THEME.card}>
              ...
              <div className="p-4 space-y-4">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[
                    { id: 'modern', name: 'Modern', color: 'from-indigo-500 to-purple-600' },
                    { id: 'classic', name: 'Classic', color: 'from-emerald-500 to-teal-600' },
                    { id: 'minimal', name: 'Minimal', color: 'from-slate-600 to-slate-700' },
                  ].map((d) => (
                    <button
                      key={d.id}
                    ...
                    >
                      <div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${d.color} mb-2 mx-auto`} />
                      <p className="text-xs font-semibold text-slate-700 text-center">{d.name}</p>
                    </button>
                  ))}
                </div>
        ...
        <div className="col-span-7">
          {showCard && generatedCard && securityFeatures ? (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="font-bold text-slate-900 text-lg">ID Card Preview</h3>
              ...
              <div className="flex gap-6 justify-center">
                {/* Front of Card */}
                <div id="modern-idcard" className="relative w-[340px] h-[215px] rounded-2xl shadow-2xl overflow-hidden">
                  ...
                  <div className="relative h-full p-5 flex flex-col text-white">
                    ...
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-2">
                        <div className="w-8 h-8 rounded-lg bg-white/20 flex items-center justify-center backdrop-blur-sm">
                          <img src={logo} alt="BMI" className="w-5 h-5 object-contain brightness-0 invert" />
                        </div>
                        <div>
                          <p className="text-xs font-bold leading-tight">BMI UNIVERSITY</p>
                    ...
                    <div className="flex gap-4 flex-1">
                      ...
                      <div className="flex-1 space-y-1">
                        <p className="text-[10px] opacity-70 uppercase tracking-wider">Student Name</p>
                        <p className="text-sm font-bold truncate">{generatedCard.studentName}</p>
                        
                        <p className="text-[10px] opacity-70 uppercase tracking-wider mt-2">ID Number</p>
                        <p className="text-xs font-mono">{generatedCard.studentNumber}</p>
                        
                        <p className="text-[10px] opacity-70 uppercase tracking-wider mt-2">Program</p>
                        <p className="text-xs truncate">{generatedCard.program}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between mt-auto pt-3 border-t border-white/20">
                      <div className="text-[9px] opacity-70">
                        <p>Valid Until: {new Date(generatedCard.expiryDate).toLocaleDateString()}</p>
                      </div>
                ...
                <div className="relative w-[340px] h-[215px] rounded-2xl shadow-2xl overflow-hidden">
                  ...
                  <div className="relative h-full p-5 flex flex-col text-white">
                    <div className="flex-1 space-y-3">
                      <div>
                        <p className="text-[10px] opacity-70 uppercase tracking-wider">Emergency Contact</p>
                        <p className="text-sm font-semibold">{generatedCard.emergencyContact}</p>
                      </div>
                      
                      <div>
                        <p className="text-[10px] opacity-70 uppercase tracking-wider">Blood Group</p>
                        <p className="text-sm font-semibold">{generatedCard.bloodGroup}</p>
                      </div>
                      
                      <div>
                        <p className="text-[10px] opacity-70 uppercase tracking-wider">Department</p>
                        <p className="text-sm">{generatedCard.department}</p>
                      </div>
                      
                      <div>
                        <p className="text-[10px] opacity-70 uppercase tracking-wider">Faculty</p>
                    ...
                    <div className="mt-auto pt-3 border-t border-white/20">
                      <p className="text-[8px] opacity-60 text-center leading-relaxed">
                        This card is the property of BMI University.<br/>
                        If found, please return to the Security Office.<br/>
                        <span className="font-mono">{securityFeatures.serialNumber}</span>
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <div className={`${THEME.card} h-full min-h-[400px] flex flex-col items-center justify-center p-12 text-center`}>
              <div className="w-20 h-20 rounded-full bg-slate-100 flex items-center justify-center mb-4">
                <IdCardIcon className="text-slate-400" size={32} />
              </div>
              <h3 className="text-xl font-bold text-slate-900 mb-2">No ID Card Generated</h3>
              <p className="text-slate-500 max-w-md">
                Select a student from the list and choose a design to generate a secure student ID card.
              </p>
            </div>
          )}
        </div>
