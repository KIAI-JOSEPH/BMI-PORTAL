...
interface QRData { qrCode: string; url: string; mode: string; }
interface CertificatesProps { students: Student[]; logo: string; }

// ── Faculty → dept code map ───────────────────────────────────────────────────
const DEPT_CODE: Record<string, string> = {
  Theology: 'THE', ICT: 'ICT', Business: 'BUS', Education: 'EDU', General: 'GEN',
};
...
function shortHash(hash: string): string {
  const h = hash.toUpperCase().replace(/[^A-F0-9]/g, '').substring(0, 20);
  ...
  <div className="absolute inset-0 pointer-events-none overflow-hidden z-10">
    ...
    <div className="absolute top-0 left-0 w-full h-3 overflow-hidden">
      <p className="text-[3.5px] leading-none whitespace-nowrap text-[#4B0082] opacity-40 tracking-[0.05em]">
        {Array(300).fill('BMI UNIVERSITY · NAIROBI KENYA · CHARTERED 2005 · CUE ACCREDITED · VERIFY AT WWW.BMI.AC.KE/VERIFY · ').join('')}
      </p>
    </div>
    {/* Bottom micro-text strip */}
    <div className="absolute bottom-0 left-0 w-full h-3 overflow-hidden">
      <p className="text-[3.5px] leading-none whitespace-nowrap text-[#4B0082] opacity-40 tracking-[0.05em]">
        {Array(300).fill('ANY ALTERATION RENDERS THIS DOCUMENT VOID · DOCUMENT SECURITY FEATURE · BMI UNIVERSITY OFFICIAL CERTIFICATE · ').join('')}
      </p>
    </div>
    {/* Left micro-text strip */}
    <div className="absolute top-0 left-0 h-full w-3 overflow-hidden flex items-center justify-center">
      <p className="text-[3.5px] leading-none whitespace-nowrap text-[#4B0082] opacity-40" style={{ writingMode: 'vertical-rl', transform: 'rotate(180deg)' }}>
        {Array(200).fill('BMI UNIVERSITY SECURE DOCUMENT · EXCELLENCE IN FAITH AND KNOWLEDGE · ').join('')}
      </p>
    </div>
    {/* Right micro-text strip */}
    <div className="absolute top-0 right-0 h-full w-3 overflow-hidden flex items-center justify-center">
      <p className="text-[3.5px] leading-none whitespace-nowrap text-[#4B0082] opacity-40" style={{ writingMode: 'vertical-rl' }}>
        {Array(200).fill('ANTI-TAMPER SECURITY LAYER · CRYPTOGRAPHICALLY SIGNED · BMI UNIVERSITY KENYA · ').join('')}
      </p>
    </div>
  </div>
);
...
const GuillocheWatermark: React.FC<{ logo: string }> = ({ logo }) => (
  <div className="absolute inset-0 pointer-events-none z-0 flex items-center justify-center opacity-[0.04]">
    <img src={logo} className="w-[420px] h-[420px] object-contain grayscale" alt="" />
  </div>
);
...
const CertificateDocument: React.FC<CertDocProps> = ({ student, cert, qrData, logo, orientation }) => {
  const serial = formatSerial(cert.serial_number);
  const hash = shortHash(cert.content_hash);
  const sig = cert.signature ? cert.signature.substring(0, 20).toUpperCase() : '';
  ...
  const timestamp = new Date(cert.issue_date).toISOString().replace('T', ' ').substring(0, 19) + 'Z';
  const verifyUrl = qrData?.url || `https://bmi.ac.ke/verify?id=${cert.serial_number}`;
  ...
  return (
    ...
    >
      ...
      {[['top-4 left-4', ''], ['top-4 right-4', 'rotate-90'], ['bottom-4 left-4', '-rotate-90'], ['bottom-4 right-4', 'rotate-180']].map(([pos, rot], i) => (
        <div key={i} className={`absolute ${pos} w-10 h-10 pointer-events-none z-20`}>
          <svg viewBox="0 0 40 40" className={`w-full h-full ${rot}`}>
            <path d="M2,2 L2,16 M2,2 L16,2" stroke="#C9A84C" strokeWidth="2.5" fill="none" />
            <path d="M2,2 L10,10" stroke="#4B0082" strokeWidth="1" fill="none" />
      ...
      {/* ── Main content ── */}
      <div className={`relative z-10 flex flex-col justify-between h-full ${isLandscape ? 'px-14 py-6' : 'px-12 py-8'}`}>

        {/* ── Header: Logo + University name ── */}
        <div className="flex flex-col items-center text-center mb-3">
          <div className="relative mb-2">
            <img src={logo} alt="BMI University" className="h-14 w-14 object-contain rounded-full border-2 border-[#C9A84C] bg-white shadow-md" />
          </div>
          <h1 className="text-[22px] font-black tracking-[0.35em] text-[#1a1a3e] uppercase" style={{ fontFamily: 'Georgia, serif', letterSpacing: '0.3em' }}>
            BMI University
          </h1>
          <p className="text-[7px] tracking-[0.25em] text-[#555] uppercase mt-0.5">
            Nairobi, Kenya &nbsp;·&nbsp; Chartered 2005 &nbsp;·&nbsp; CUE Accredited
          </p>
          {/* Gold divider */}
        ...
        <div className="flex items-center justify-center gap-3 mb-3">
          <div className="flex items-center gap-2 bg-[#1a1a3e] text-white px-3 py-1 rounded-sm text-[7px] tracking-widest uppercase">
            ...
            <span className="font-mono text-gray-300">{hash.substring(0, 12)}</span>
        ...
        <div className="text-center mb-2">
          <h2 className="text-[18px] text-[#1a1a3e] italic" style={{ fontFamily: 'Georgia, serif' }}>
        ...
        {/* ── Body text ── */}
        <div className="text-center mb-2">
          <p className="text-[9px] text-[#444] tracking-wide">The Senate and Council of BMI University hereby certify that</p>
        </div>

        {/* ── Student name ── */}
        <div className="text-center mb-1">
          <h3 className="text-[26px] font-bold text-[#1a1a3e]" style={{ fontFamily: 'Georgia, serif' }}>
            {student.firstName} {student.lastName}
          </h3>
          <div className="inline-block border border-[#4B0082]/40 px-3 py-0.5 mt-1 rounded-sm">
            <span className="text-[7.5px] font-mono text-[#4B0082] tracking-widest">{studentRef}</span>
          </div>
        </div>

        {/* ── Degree text ── */}
        <div className="text-center mb-2">
          <p className="text-[9px] text-[#444] tracking-wide">having satisfactorily fulfilled all requirements prescribed by the Senate,</p>
          <p className="text-[9px] text-[#444] tracking-wide">is hereby admitted to the degree of</p>
        </div>

        {/* ── Degree title box ── */}
        <div className="flex justify-center mb-2">
          <div className="border-2 border-[#C9A84C] rounded-sm px-8 py-2 bg-[#FFFBF0]">
            <p className="text-[14px] font-bold text-[#8B6914] text-center" style={{ fontFamily: 'Georgia, serif' }}>
              {cert.degree}
            </p>
          </div>
        </div>
        ...
        {/* ── GPA + Class ── */}
        <div className="text-center mb-2">
          <p className="text-[8.5px] text-[#444]">with a Cumulative Grade Point Average of</p>
          <div className="flex items-baseline justify-center gap-2 mt-0.5">
            <span className="text-[22px] font-black text-[#1a1a3e]" style={{ fontFamily: 'Georgia, serif' }}>
          ...
          {cert.graduation_class && (
            <div className="inline-flex items-center gap-1.5 bg-[#4B0082] text-white px-3 py-0.5 rounded-full mt-1">
              <span className="text-[7px] tracking-widest uppercase">{cert.graduation_class}</span>
            </div>
          )}
          <p className="text-[8px] text-[#555] mt-1">Date of Issue: <strong>{issueDate}</strong></p>
        </div>
        ...
        <div className="grid grid-cols-3 items-end gap-4 mb-3">
          ...
          <div className="flex flex-col items-start">
            <div className="relative w-28 mb-1">
              <p className="font-[cursive] text-[13px] text-[#1a1a3e] italic -mb-1 ml-2">Prof. I. Sigei</p>
              <div className="h-[1px] bg-gray-700 w-full" />
            </div>
            <p className="text-[7.5px] font-bold text-[#1a1a3e] tracking-wide">Prof. Isaac Sigei, PhD</p>
          ...
          <div className="flex flex-col items-center">
            <div className="relative w-16 h-16">
              ...
              <div className="absolute inset-[3px] rounded-full bg-[#4B0082] flex items-center justify-center overflow-hidden border-2 border-[#C9A84C]/60">
                <img src={logo} alt="Seal" className="w-10 h-10 object-contain filter brightness-0 invert opacity-90" />
              </div>
              {/* Seal text ring */}
              <svg className="absolute inset-0 w-full h-full" viewBox="0 0 64 64">
                <path id="sealArc" d="M 32,6 A 26,26 0 1,1 31.9,6" fill="none" />
                <text className="text-[4px]" fill="#C9A84C" fontSize="4.5" fontWeight="bold" letterSpacing="1.5">
                  <textPath href="#sealArc" startOffset="5%">UNIVERSITY SEAL · BMI · EST. 2015</textPath>
                </text>
              </svg>
            </div>
            <p className="text-[6px] tracking-[0.2em] uppercase text-[#888] mt-1">University Seal</p>
          </div>

          {/* Academic Registrar */}
          <div className="flex flex-col items-end">
            <div className="relative w-28 mb-1">
              <p className="font-[cursive] text-[13px] text-[#1a1a3e] italic -mb-1 mr-2 text-right">Dr. S. Kiptoo</p>
              <div className="h-[1px] bg-gray-700 w-full" />
            </div>
            <p className="text-[7.5px] font-bold text-[#1a1a3e] tracking-wide text-right">Dr. Samuel Kiptoo</p>
            <p className="text-[6.5px] text-[#666] text-right">Academic Registrar</p>
            <p className="text-[6px] text-[#888] font-mono text-right">REG/BMI/001</p>
          </div>
        </div>

        {/* ── Security footer (dark bar) ── */}
        <div className="bg-[#1a1a3e] text-white px-4 py-2 -mx-14 grid grid-cols-3 items-center gap-2" style={{ marginLeft: isLandscape ? '-3.5rem' : '-3rem', marginRight: isLandscape ? '-3.5rem' : '-3rem' }}>
          {/* Left: hash + timestamp */}
          <div>
            <p className="text-[6px] font-bold text-[#C9A84C] uppercase tracking-widest mb-0.5">Document Security Hash</p>
            <p className="text-[6.5px] font-mono text-white tracking-wider">{hash}</p>
            <p className="text-[5.5px] text-gray-400 mt-0.5">
              TIMESTAMP: {timestamp} · REF: {serial} · SERIAL: {cert.serial_number.split('-')[2]}
            </p>
          </div>
          ...
          <div className="flex flex-col items-center">
            {qrData?.qrCode ? (
              <img src={qrData.qrCode} alt="Scan to Verify" className="w-12 h-12 bg-white p-0.5 rounded-sm" />
            ) : (
              <div className="w-12 h-12 bg-white/10 flex items-center justify-center rounded-sm">
                <QrCode size={24} className="text-white/40" />
              </div>
            )}
            <p className="text-[5.5px] tracking-widest uppercase text-gray-400 mt-0.5">Scan to Verify</p>
          </div>

          {/* Right: verify instructions */}
          <div className="text-right">
            <p className="text-[6px] font-bold text-[#C9A84C] uppercase tracking-widest mb-0.5">Verify Authenticity</p>
            <p className="text-[6px] text-white">www.bmi.ac.ke/verify</p>
            <p className="text-[6px] text-gray-300">registrar@bmiuniversity.org</p>
            <p className="text-[6px] text-gray-300">+254 726 912 577</p>
            <p className="text-[5.5px] text-[#C9A84C] mt-0.5 font-bold uppercase tracking-wide">Any alteration renders this document void</p>
          </div>
        </div>
...
const Certificates: React.FC<CertificatesProps> = ({ students, logo }) => {
  ...
  const handleGenerate = async (student: Student) => {
    ...
    try {
      ...
      const numericId = student.id.replace(/\D/g, '').slice(-6).padStart(6, '0');
    ...
    <div className="h-full flex flex-col animate-fade-in">
      ...
      <div className="flex-shrink-0 sticky top-0 z-40 bg-white/95 dark:bg-gray-900/95 backdrop-blur-md border-b border-gray-200 dark:border-gray-700 px-4 py-2 flex justify-between items-center shadow-sm min-h-[60px]">
        <div className="flex items-center gap-3 pl-14">
          ...
          <div>
            <h2 className="text-base font-bold text-[#2E004F] dark:text-white tracking-tight uppercase">Degree & Certificate Issuance</h2>
            <p className="text-[8px] font-semibold text-gray-500 uppercase tracking-widest">BMI Institutional Registrar · Graduation & Awards</p>
          </div>
        </div>
        <div className="flex items-center gap-2 pr-4">
          {(['landscape', 'portrait'] as const).map(o => (
            <button key={o} onClick={() => setOrientation(o)}
              ...
              <Layout size={10} className="inline mr-1" />{o}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-1 bg-white dark:bg-gray-800 p-6 border border-gray-100 dark:border-gray-700 shadow-sm flex flex-col h-[600px]">
            <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 mb-4">Candidate Registry</h3>
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar space-y-1">
              {filteredStudents.map(s => (
                <button key={s.id}
                  onClick={() => { setSelectedStudent(s); setShowCertificate(false); setGeneratedCert(null); setError(''); }}
                  ...
                  <div>
                    <p className="text-[11px] font-black uppercase tracking-tight">{s.firstName} {s.lastName}</p>
                    <p className={`text-[9px] font-bold uppercase mt-0.5 ${selectedStudent?.id === s.id ? 'text-purple-200' : 'text-gray-400'}`}>{s.id}</p>
                  </div>
                  <ChevronRight size={13} className={selectedStudent?.id === s.id ? 'text-[#FFD700]' : 'text-gray-300'} />
                </button>
              ))}
            </div>
          </div>
            ...
            {selectedStudent ? (
              <div className="space-y-4">
                <div className="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 p-6 flex justify-between items-center relative overflow-hidden shadow-sm">
                  ...
                  <div className="pl-2">
                    <h3 className="text-xl font-black text-gray-900 dark:text-white uppercase">{selectedStudent.firstName} {selectedStudent.lastName}</h3>
                    <p className="text-xs font-bold text-[#4B0082] dark:text-[#FFD700] uppercase tracking-widest mt-1">{getDegreeTitle(selectedStudent)}</p>
                    <p className="text-[10px] text-gray-400 mt-0.5">{selectedStudent.id} · GPA: {selectedStudent.gpa} · {getGraduationClass(selectedStudent)}</p>
                  </div>
                  <button onClick={() => handleGenerate(selectedStudent)} disabled={isGenerating}
                    ...
                    {isGenerating ? <><Loader2 size={14} className="animate-spin" />Generating...</> : <><Award size={14} className="text-[#FFD700]" />Issue Certificate</>}
                  </button>
                </div>
                ...
                {!showCertificate && !error && (
                  <div className="bg-gray-50 dark:bg-gray-900 border-2 border-dashed border-gray-200 dark:border-gray-700 p-16 flex flex-col items-center justify-center text-center min-h-[400px]">
                    ...
                    <h4 className="text-base font-black text-gray-300 uppercase tracking-widest mb-2">Secure Certificate Issuance</h4>
                    <p className="text-xs text-gray-400 max-w-sm">Click "Issue Certificate" to generate a cryptographically signed certificate stored in the database with a verifiable QR code.</p>
                  </div>
                )}
              </div>
            ) : (
              <div className="h-full min-h-[600px] flex flex-col items-center justify-center bg-white dark:bg-gray-800 border-2 border-dashed border-gray-100 dark:border-gray-700 text-gray-400">
                ...
                <h3 className="text-lg font-black uppercase tracking-[0.3em] opacity-40">Select Candidate</h3>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="flex flex-col items-center gap-4">
            ...
            <div className="w-full max-w-[297mm] flex justify-between items-center bg-gray-900 px-6 py-4 text-white no-print border-t border-white/10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-[9px] font-black uppercase tracking-widest text-gray-400">Issued · Signed · Verified</p>
                  <p className="text-[8px] text-[#FFD700] font-mono mt-0.5">{formatSerial(generatedCert.serial_number)}</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => { setShowCertificate(false); setGeneratedCert(null); setQrData(null); }}
